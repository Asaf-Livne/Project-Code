...
def load_data(clean_path, fx_path, sr=48000, batch_size = 5, bit_size=5):
    ...
    number_of_splits = data_len // sr // bit_size
    split_clean = [[clean_data[i*sr*bit_size:(i+1)*sr*bit_size]] for i in range(number_of_splits)]
    split_fx = [[fx_data[i*sr*bit_size:(i+1)*sr*bit_size]] for i in range(number_of_splits)]
